...
export default function ExitIntent() {
  ...
  useEffect(() => {
    const onMouseOut = (event) => {
      if (event.clientY <= 4 && !sessionStorage.getItem("atomic-exit-seen")) {
        sessionStorage.setItem("atomic-exit-seen", "1");
        setOpen(true);
      }
    };
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-foreground/40 p-4 backdrop-blur-sm">
      <Card className="relative w-full max-w-md p-8 text-center">
        ...
        <span className="mx-auto grid size-12 place-items-center rounded-xl bg-[var(--accent-atomic)] text-[var(--accent-atomic-foreground)]">
          <Gift />
        </span>
        ...
        <h2 className="mt-3 font-heading text-2xl font-semibold">
          Wait! Get 30% off your first year.
        </h2>
        <p className="mt-3 text-sm text-muted-foreground">
          Make the next step easier to start.
        </p>
        {claimed ? (
          <p className="mt-5 flex items-center justify-center gap-2 text-sm text-emerald-600">
            <CheckCircle2 className="size-4" />
            Offer reserved. Check your inbox.
          </p>
        ) : (
          <form
