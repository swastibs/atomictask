...
export function SectionFrame({ id, className = "", children }) {
  ...
  return (
    <section id={id} ref={ref} className={`border-b border-border/70 px-4 py-20 transition duration-700 motion-reduce:transform-none motion-reduce:transition-none sm:px-6 sm:py-24 ${revealClass} ${className}`}>
...
export function SectionIntro({ eyebrow, title, children }) {
  return (
    <div className="mx-auto mb-12 max-w-2xl text-center">
      <span className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">{eyebrow}</span>
...
export function DemoCard({ className = "", children }) {
  return <Card className={`border-border/70 shadow-lg ${className}`}>{children}</Card>;
}
