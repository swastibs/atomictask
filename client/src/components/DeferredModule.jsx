...
export default function DeferredModule({ children, minHeight = "min-h-96" }) {
  ...
  return <div ref={ref} className={minHeight}>{isVisible ? <Suspense fallback={<div className="grid min-h-96 place-items-center text-sm text-muted-foreground">Loading preview...</div>}>{children}</Suspense> : null}</div>;
}
