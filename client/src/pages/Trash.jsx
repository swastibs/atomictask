...
export default function Trash() {
  ...
  useEffect(load, []);
  const action = async (task, permanent) => {
    ...
    try {
      await (permanent
        ? taskApi.permanentRemove(task._id)
        : taskApi.restore(task._id));
      load();
    } catch (requestError) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6 lg:py-10">
      ...
      <header>
        ...
        <h1 className="mt-2 font-heading text-3xl font-semibold">Trash</h1>
        <p className="mt-2 text-muted-foreground">
          Deleted tasks remain recoverable until permanently removed.
        </p>
      </header>
      {error && (
        <p
